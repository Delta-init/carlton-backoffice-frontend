...
export default function Exchangers() {
  ...
  const fetchExchangerTransactions = async (vendorId) => {
    try {
      const response = await fetch(
        `${API_URL}/api/vendors/${vendorId}/transactions`,
  ...
  const fetchExchangerSettlements = async (vendorId) => {
    try {
      const response = await fetch(
        `${API_URL}/api/vendors/${vendorId}/settlements`,
        { headers: getAuthHeaders(), credentials: "include" },
      );
      if (response.ok) {
  ...
  const fetchVendorIeEntries = async (vendorId) => {
    try {
      const response = await fetch(
        `${API_URL}/api/income-expenses?vendor_id=${vendorId}&page_size=100`,
        { headers: getAuthHeaders(), credentials: "include" },
      );
      if (response.ok) {
        const data = await response.json();
  ...
  const fetchVendorLoanTransactions = async (vendorId) => {
    try {
      const response = await fetch(
        `${API_URL}/api/loans/transactions?vendor_id=${vendorId}&page_size=100`,
        { headers: getAuthHeaders(), credentials: "include" },
      );
      if (response.ok) {
        const data = await response.json();
        // Handle paginated response
        setVendorLoanTxs(
          Array.isArray(data) ? data : data.items || data.transactions || [],
        );
      }
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...
        deposit_commission_cash:
          parseFloat(formData.deposit_commission_cash) || 0,
        withdrawal_commission_cash:
          parseFloat(formData.withdrawal_commission_cash) || 0,
      ...
      if (response.ok) {
        toast.success(
          selectedExchanger ? "Exchanger updated" : "Exchanger created",
        );
  ...
  const handleEdit = (vendor) => {
    ...
    setFormData({
      ...
      withdrawal_commission_cash:
        vendor.withdrawal_commission_cash?.toString() || "",
  ...
  const handleSettleExchanger = async () => {
    ...
    const destAccount = treasuryAccounts.find(
      (a) => a.account_id === settlementDestination,
    );
    ...
    try {
      ...
      const baseCurrencyData = viewExchanger?.settlement_by_currency?.find(
        (c) => c.currency === baseCurrency,
      );
      const preCalculatedCommission =
        baseCurrencyData?.commission_earned_base || 0;

      const response = await fetch(
        `${API_URL}/api/vendors/${viewExchanger.vendor_id}/settle`,
        {
          method: "POST",
          headers: getAuthHeaders(),
          credentials: "include",
          body: JSON.stringify({
            ...
            settlement_amount_in_dest_currency: settlementAmountInDestCurrency
              ? parseFloat(settlementAmountInDestCurrency)
              : null,
          }),
        },
      );
  ...
  const totalPendingAmount = vendors.reduce(
    (sum, v) => sum + (v.pending_amount || 0),
    0,
  );
  const pendingApproved = pendingTransactions.filter(
    (t) => t.status === "approved" && !t.settled,
  );
    ...
    <div className="space-y-6 animate-fade-in" data-testid="vendors-page">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500">
            Manage exchangers, commissions, and settlements
          </p>
        </div>
        {isAccountantOrAdmin && (
          <Dialog
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="space-y-3">
                  ...
                  <div className="grid grid-cols-3 gap-2 items-center">
                    <div></div>
                    <Label className="text-center text-xs text-blue-600 uppercase">
                      Bank
                    </Label>
                    <Label className="text-center text-xs text-amber-600 uppercase">
                      Cash
                    </Label>
                  </div>
                  <div className="grid grid-cols-3 gap-2 items-center">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setFormData({
                          ...formData,
                          deposit_commission: e.target.value,
                        })
                      }
                    ...
                    <Input
                      type="number"
                      step="0.01"
                      value={formData.deposit_commission_cash}
                      onChange={(e) =>
                        setFormData({
                          ...formData,
                          deposit_commission_cash: e.target.value,
                        })
                      }
                      className="bg-slate-50 border-slate-200 text-slate-800 font-mono"
                      placeholder="0"
                      data-testid="vendor-deposit-commission-cash"
                    />
                  </div>
                  <div className="grid grid-cols-3 gap-2 items-center">
                    <Label className="text-slate-500 text-xs uppercase">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setFormData({
                          ...formData,
                          withdrawal_commission: e.target.value,
                        })
                      }
                      className="bg-slate-50 border-slate-200 text-slate-800 font-mono"
                      placeholder="0"
                    ...
                    <Input
                      type="number"
                      step="0.01"
                      value={formData.withdrawal_commission_cash}
                      onChange={(e) =>
                        setFormData({
                          ...formData,
                          withdrawal_commission_cash: e.target.value,
                        })
                      }
                      className="bg-slate-50 border-slate-200 text-slate-800 font-mono"
                      placeholder="0"
                      data-testid="vendor-withdrawal-commission-cash"
                    />
                  </div>
                </div>
          ...
          <div className="col-span-full text-center py-12">
            ...
            {isAccountantOrAdmin && (
              <p className="text-sm text-slate-500/60 mt-2">
                Click "Add Exchanger" to create one
              </p>
            )}
          </div>
        ) : (
                    ...
                    <DropdownMenu>
                      ...
                      >
                        <Button
                          ...
                          data-testid={`vendor-actions-${vendor.vendor_id}`}
                        >
              ...
              <CardContent>
                <div className="space-y-2">
                  <div className="grid grid-cols-3 gap-1 text-xs">
                    ...
                    <span className="text-center text-amber-600 font-semibold">
                      Cash
                    </span>
                  </div>
                  <div className="grid grid-cols-3 gap-1 items-center">
                    ...
                    <span className="text-slate-800 font-mono text-center">
                      {vendor.deposit_commission || 0}%
                    </span>
                    <span className="text-slate-800 font-mono text-center">
                      {vendor.deposit_commission_cash || 0}%
                    </span>
                  </div>
                  <div className="grid grid-cols-3 gap-1 items-center">
                    <span className="text-slate-500 text-sm flex items-center gap-1">
                    ...
                    <span className="text-slate-800 font-mono text-center">
                      {vendor.withdrawal_commission || 0}%
                    </span>
                    <span className="text-slate-800 font-mono text-center">
                      {vendor.withdrawal_commission_cash || 0}%
                    </span>
                  </div>
                  <div className="pt-2 border-t border-slate-200">
                    ...
                    vendor.settlement_by_currency.length > 0 ? (
                      <div className="mt-1 space-y-1">
                        ...
                        <div className="flex justify-between items-center pt-1 border-t border-slate-200">
                          ...
                          <span className="text-blue-600 font-mono font-bold">
                            ${(vendor.pending_amount || 0).toLocaleString()}
                          </span>
                        </div>
                      </div>
                    ) : (
      ...
      >
        <DialogContent className="bg-white border-slate-200 text-slate-800 max-w-4xl max-h-[90vh] overflow-hidden">
          <DialogHeader>
            <DialogTitle
              className="text-2xl font-bold uppercase tracking-tight flex items-center gap-3"
              style={{ fontFamily: "Barlow Condensed" }}
            >
              <Store className="w-6 h-6 text-blue-600" />
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 p-4 bg-slate-50 rounded-xl">
                <div>
                  ...
                  <p className="text-xl font-mono text-slate-800">
                    {viewExchanger.deposit_commission || 0}%
                  </p>
                </div>
                <div>
                  <p className="text-xs text-slate-500 uppercase tracking-wider mb-1">
                  ...
                  <p className="text-xl font-mono text-amber-600">
                    {viewExchanger.deposit_commission_cash || 0}%
                  </p>
                </div>
                <div>
                  <p className="text-xs text-slate-500 uppercase tracking-wider mb-1">
                  ...
                  <p className="text-xl font-mono text-slate-800">
                    {viewExchanger.withdrawal_commission || 0}%
                  </p>
                </div>
                <div>
                  <p className="text-xs text-slate-500 uppercase tracking-wider mb-1">
                  ...
                  <p className="text-xl font-mono text-amber-600">
                    {viewExchanger.withdrawal_commission_cash || 0}%
                  </p>
                </div>
              </div>

              {/* Settlement Balance by Currency */}
              <div className="p-4 bg-slate-50 rounded-xl border-l-4 border-l-[#66FCF1]">
                <p className="text-xs text-blue-600 uppercase tracking-wider mb-3">
                  Settlement Balance (Money In - Money Out - Commission)
                </p>
                {viewExchanger.settlement_by_currency &&
                  ...
                  <div className="space-y-3">
                      ...
                      <div key={idx} className="space-y-1">
                        <div className="flex items-center justify-between">
                          ...
                          <div className="text-right">
                            <span
                              className={`text-lg font-bold font-mono ${item.amount >= 0 ? "text-blue-600" : "text-red-400"}`}
                            >
                              {item.amount?.toLocaleString()}
                            </span>
                            {item.currency !== "USD" && (
                        ...
                        <div className="pl-2 text-xs space-y-0.5">
                          ...
                          {item.ie_in > 0 && (
                            <div className="flex justify-between text-slate-400 pl-3">
                              <span>I&E In</span>
                              <span>+{item.ie_in?.toLocaleString()}</span>
                            </div>
                          )}
                          {item.loan_in > 0 && (
                            <div className="flex justify-between text-slate-400 pl-3">
                              <span>Loan In</span>
                              <span>+{item.loan_in?.toLocaleString()}</span>
                            </div>
                          )}
                          {/* Money Out breakdown */}
                          ...
                          <div className="flex justify-between text-slate-400 pl-3">
                            ...
                            <span>
                              -{item.withdrawal_amount?.toLocaleString()}
                            </span>
                          </div>
                          {item.ie_out > 0 && (
                            <div className="flex justify-between text-slate-400 pl-3">
                              <span>I&E Out</span>
                              <span>-{item.ie_out?.toLocaleString()}</span>
                            </div>
                          )}
                          {item.loan_out > 0 && (
                            <div className="flex justify-between text-slate-400 pl-3">
                              <span>Loan Out</span>
                              <span>-{item.loan_out?.toLocaleString()}</span>
                            </div>
                          )}
                          {/* Commission */}
                    ...
                    <div className="border-t border-slate-200 pt-2 mt-2">
                      <div className="flex justify-between mb-1">
                        ...
                        <span className="text-sm font-bold font-mono text-yellow-400">
                          ...
                          {viewExchanger.settlement_by_currency
                            .reduce(
                              (sum, item) =>
                                sum + (item.commission_earned_usd || 0),
                              0,
                            )
                            .toLocaleString()}
                        </span>
                      ...
                      <div className="flex justify-between">
                        ...
                        <span
                          className={`text-lg font-bold font-mono ${viewExchanger.settlement_by_currency.reduce((sum, item) => sum + (item.usd_equivalent || 0), 0) >= 0 ? "text-slate-800" : "text-red-400"}`}
                        >
                          $
                          {viewExchanger.settlement_by_currency
                            .reduce(
              ...
              <Tabs defaultValue="transactions" className="w-full">
                <TabsList className="bg-slate-50 border border-slate-200">
                  <TabsTrigger
                    value="transactions"
                    className="data-[state=active]:bg-primary data-[state=active]:text-[#0B0C10]"
                  >
                            ...
                            >
                                  ...
                                  {settlement.charges_amount > 0 && (
                                    <div>
                                      ...
                                      settlement.source_currency !== "USD"
                                        ? `${settlement.source_currency} `
                                        : "$"}
                                      {settlement.charges_amount?.toLocaleString()}
                                    </div>
                                  )}
                                </div>
                              </TableCell>
                              <TableCell className="font-mono text-green-400">
                                {settlement.destination_currency &&
                              ...
                              <TableCell className="text-right">
                                <Button
                                  ...
                                  size="sm"
                                  onClick={() =>
                                    openStatement(settlement.settlement_id)
                                  }
                      ...
                      <Table>
                        <TableHeader>
                          <TableRow className="border-slate-200">
                            <TableHead className="text-slate-500 text-xs uppercase">
                              Reference
                            </TableHead>
                            <TableHead className="text-slate-500 text-xs uppercase">
                              Type
                          ...
                          {vendorIeEntries.map((entry) => {
                            ...
                            return (
                              ...
                              >
                                ...
                                <TableCell>
                                  <span
                                    className={`flex items-center gap-1 text-xs ${isIncome ? "text-green-600" : "text-red-500"}`}
                                  >
                                    {isIncome ? (
                                      <ArrowDownRight className="w-3 h-3" />
                                    ) : (
                                ...
                                <TableCell
                                  className={`font-mono text-xs ${isIncome ? "text-green-600" : "text-red-500"}`}
                                >
                                  {isIncome ? "+" : "-"}
                                ...
                                <TableCell>
                                  ...
                                  {entry.transaction_mode === "cash" &&
                                    entry.collecting_person_name && (
                                      <div className="text-[10px] text-slate-600 mt-0.5 space-y-0.5">
                                        <p className="font-medium">
                                          {entry.collecting_person_name}
                                        </p>
                                        {entry.collecting_person_number && (
                                          <p className="text-slate-500">
                                            {entry.collecting_person_number}
                                          </p>
                                        )}
                      ...
                      <Table>
                        ...
                        <TableBody>
                          {vendorLoanTxs.map((tx) => {
                            ...
                            const isVendorSource =
                              tx.source_vendor_id === viewExchanger?.vendor_id;
                            return (
                              <TableRow
                                key={tx.transaction_id}
                                className="border-slate-200 hover:bg-slate-100"
                              >
                                ...
                                <TableCell>
                                  <span
                                    className={`flex items-center gap-1 ${isVendorSource ? "text-red-400" : "text-green-400"}`}
                                  >
                                    {isVendorSource ? (
                                      <ArrowUpRight className="w-3 h-3" />
                                    ) : (
                                ...
                                <TableCell
                                  className={`font-mono ${isVendorSource ? "text-red-400" : "text-green-400"}`}
                                >
                                  {isVendorSource ? "-" : "+"}
            ...
            <div className="space-y-4">
              <div className="p-4 bg-slate-50 rounded-xl space-y-3">
                ...
                {viewExchanger?.settlement_by_currency?.map((item, idx) => (
                  <div
                    key={idx}
                    className="border-t border-slate-200 pt-2 space-y-1"
                  >
                    ...
                    <div className="text-xs space-y-0.5 pl-1">
                      <div className="flex justify-between text-green-500">
                        <span>Money In (Dep + I&E + Loan)</span>
                        <span>+{item.total_in?.toLocaleString()}</span>
                      </div>
                      <div className="flex justify-between text-red-400">
                        <span>Money Out (Wdr + I&E + Loan)</span>
                        <span>-{item.total_out?.toLocaleString()}</span>
                      </div>
                      <div className="flex justify-between text-yellow-400">
                        <span>Commission</span>
              ...
              {settlementCharges && parseFloat(settlementCharges) > 0 && (
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      setSettlementChargesDescription(e.target.value)
                    }
                    className="bg-slate-50 border-slate-200 text-slate-800 focus:border-[#66FCF1]"
              ...
              {settlementDestination &&
                (() => {
                  ...
                  const isSameCurrency = destCurrency === baseCurrency;
                  const finalAmountDest = isSameCurrency
                    ? netSettlementBase - additionalChargesDest
                    : null;
                      ...
                      <div className="space-y-2 text-sm">
                        ...
                        <div className="flex justify-between items-center">
                          ...
                          <span className="text-slate-800 font-mono">
                            {netSettlementBase.toLocaleString()} {baseCurrency}
                          </span>
                        </div>
                        ...
                        {additionalChargesDest > 0 && (
                          <div className="flex justify-between">
                            ...
                            <span className="text-red-400 font-mono">
                              -{additionalChargesDest.toLocaleString()}{" "}
                              {destCurrency}
                            </span>
                          </div>
                        )}
                        ...
                        {isSameCurrency ? (
                          <div className="flex justify-between pt-2 border-t border-slate-200">
                            ...
                            <span className="text-blue-600 font-mono font-bold">
                              {finalAmountDest?.toLocaleString()} {destCurrency}
                            </span>
                          </div>
                        ) : (
                          <div className="pt-3 border-t border-slate-200 space-y-2">
                            <p className="text-xs text-yellow-400">
                              Destination: {destCurrency} (different from{" "}
                              {baseCurrency})
                            </p>
                            <div className="space-y-1">
                              <Label className="text-slate-500 text-xs">
                                Final Settlement Amount in {destCurrency} *
                              </Label>
                              <Input
                                type="number"
                                step="0.01"
                    ...
                    <div id="settlement-statement" className="px-8 py-6">
                      ...
                      {ieEntries.length > 0 && (
                        <>
                          ...
                          >
                            Income & Expense Entries ({ieEntries.length})
                          </h4>
                          <table
                            style={{
                              width: "100%",
                              ...
                              {ieEntries.map((ie, idx) => {
                                ...
                                return (
                                  ...
                                  >
                                    ...
                                    >
                                      {ie.entry_id?.slice(-10)?.toUpperCase()}
                                    </td>
                                    <td
                                      style={{
                                        padding: "8px 12px",
                      ...
                      {loanEntries.length > 0 && (
                        <>
                          ...
                          >
                            Loan Transactions ({loanEntries.length})
                          </h4>
                          <table
                            style={{
                              width: "100%",
                      ...
                      >
                        ...
                        >
                          <span>
                            Money In ({deposits.length} deposits
                            {ieIn.length ? ` + ${ieIn.length} I&E` : ""}
                            {loanIn.length ? ` + ${loanIn.length} loans` : ""})
                          </span>
                        ...
                        >
                          <span>
                            Money Out ({withdrawals.length} withdrawals
                            {ieOut.length ? ` + ${ieOut.length} I&E` : ""}
                            {loanOut.length ? ` + ${loanOut.length} loans` : ""}
                            )
                          </span>
                          <span style={{ fontFamily: "monospace" }}>
                            -
