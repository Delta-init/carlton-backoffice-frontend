  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2 text-[#1FA21B]">
        ...
        <span className="text-sm uppercase tracking-wider font-semibold">Security Verification</span>
      </div>
      <div className="p-4 bg-slate-50 rounded border border-slate-200">
        <p className="text-slate-600 text-sm mb-3">Solve this math problem to confirm reinstatement:</p>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="flex gap-3">
          <Button type="button" variant="outline" onClick={onCancel} className="flex-1 border-slate-200 text-slate-600">
            Cancel
          </Button>
  ...
  return (
    <div className="grid grid-cols-3 gap-2 items-center py-2 border-b border-slate-100 last:border-0">
      ...
      <div className="text-right">
        <span className="text-slate-700 text-xs font-mono">{fmt(before, currency)}</span>
        <p className="text-slate-400 text-[10px]">Before</p>
      </div>
      <div className="text-right">
        <span className={`text-xs font-mono font-semibold ${isPositive ? "text-green-600" : isNegative ? "text-red-600" : "text-slate-700"}`}>
          {fmt(after, currency)}
        </span>
        <p className="flex items-center justify-end gap-0.5 text-[10px]">
          {isPositive ? <TrendingUp className="w-3 h-3 text-green-500" /> : isNegative ? <TrendingDown className="w-3 h-3 text-red-500" /> : <Minus className="w-3 h-3 text-slate-400" />}
          <span className={isPositive ? "text-green-600" : isNegative ? "text-red-600" : "text-slate-400"}>
            {isPositive ? "+" : ""}{fmt(change, currency)}
          </span>
        </p>
      </div>
    </div>
  ...
  return (
    <div className="space-y-3 max-h-[55vh] overflow-y-auto pr-1">
      ...
      <div className="bg-slate-50 rounded border border-slate-200 p-3">
        <p className="text-[#1FA21B] text-xs uppercase tracking-wider font-semibold mb-2">Record Being Reinstated</p>
        <div className="grid grid-cols-2 gap-x-4 gap-y-1">
          {(() => {
            ...
            return entries.map(([k, v]) => (
              <div key={k}>
                <p className="text-slate-400 text-[10px] uppercase tracking-wider">{k.replace(/_/g, " ")}</p>
                <p className="text-slate-800 text-xs font-mono truncate">{String(v ?? "-")}</p>
              </div>
            ));
          })()}
        </div>
      ...
      {balance_changes.length > 0 && (
        <div className="bg-slate-50 rounded border border-slate-200 p-3">
          <p className="text-[#1FA21B] text-xs uppercase tracking-wider font-semibold mb-3">Balance Changes</p>
          <div className="grid grid-cols-3 gap-2 mb-1">
            <span />
            <span className="text-slate-400 text-[10px] text-right uppercase">Before</span>
            <span className="text-slate-400 text-[10px] text-right uppercase">After</span>
          </div>
          {balance_changes.map((bc, i) => (
            <div key={i}>
              <div className="flex items-center gap-1.5 mb-1">
                <Badge className={`text-[10px] px-1.5 py-0 ${bc.type === "psp" ? "bg-purple-100 text-purple-700 border-purple-200" : "bg-blue-100 text-blue-700 border-blue-200"}`}>
                  {bc.type === "psp" ? "PSP" : "Treasury"}
                </Badge>
                <span className="text-slate-700 text-xs font-semibold">{bc.account_name}</span>
              </div>
              <BalanceRow label={bc.description} before={bc.balance_before} after={bc.balance_after} currency={bc.currency} />
            </div>
          ))}
        </div>
      )}
      ...
      {vendor_stats && (
        <div className="bg-slate-50 rounded border border-slate-200 p-3">
          <p className="text-[#1FA21B] text-xs uppercase tracking-wider font-semibold mb-2">Exchanger Stats — {vendor_stats.vendor_name}</p>
          <BalanceRow label="Total Volume" before={vendor_stats.total_volume_before} after={vendor_stats.total_volume_after} currency="" />
          <BalanceRow label="Total Commission" before={vendor_stats.total_commission_before} after={vendor_stats.total_commission_after} currency="" />
        </div>
      )}
      ...
      {psp_changes && (
        <div className="bg-slate-50 rounded border border-slate-200 p-3">
          <p className="text-[#1FA21B] text-xs uppercase tracking-wider font-semibold mb-2">PSP Stats — {psp_changes.psp_name}</p>
          <BalanceRow label="Pending Settlement" before={psp_changes.pending_settlement_before} after={psp_changes.pending_settlement_after} currency={psp_changes.currency} />
          <BalanceRow label="Total Volume" before={psp_changes.total_volume_before} after={psp_changes.total_volume_after} currency={psp_changes.currency} />
        </div>
      )}
      ...
      {loan_changes && (
        <div className="bg-slate-50 rounded border border-slate-200 p-3">
          <p className="text-[#1FA21B] text-xs uppercase tracking-wider font-semibold mb-2">Loan Impact — {loan_changes.loan_id}</p>
          <BalanceRow label="Total Repaid" before={loan_changes.total_repaid_before} after={loan_changes.total_repaid_after} currency={loan_changes.currency} />
          <BalanceRow label="Outstanding Balance" before={loan_changes.outstanding_before} after={loan_changes.outstanding_after} currency={loan_changes.currency} />
          <div className="grid grid-cols-3 gap-2 items-center py-2">
            <span className="text-slate-500 text-xs">Repayment Count</span>
            <span className="text-slate-700 text-xs font-mono text-right">{loan_changes.repayment_count_before}</span>
      ...
      {affected_records.length > 0 && (
        <div className="space-y-1.5">
          {affected_records.map((rec, i) => (
            <div key={i} className={`flex items-start gap-2 p-2.5 rounded border text-xs ${rec.type === "warning" ? "bg-yellow-50 border-yellow-200 text-yellow-700" : "bg-slate-50 border-slate-200 text-slate-600"}`}>
              {rec.type === "warning" && <AlertTriangle className="w-3.5 h-3.5 text-yellow-500 shrink-0 mt-0.5" />}
              {rec.description}
            </div>
          ))}
        </div>
...
const tabConfig = [
  {
    ...
    endpoint: "transactions", idField: "transaction_id",
    displayFields: (item) => {
  ...
  {
    key: "vendor-settlements", label: "Vendor Settlements", icon: Store,
    endpoint: "vendor-settlements", idField: "settlement_id",
    displayFields: (item) => [
  ...
  {
    key: "income-expenses", label: "Income / Expenses", icon: Wallet,
    endpoint: "income-expenses", idField: "entry_id",
    displayFields: (item) => [
  ...
  },
  {
    ...
    endpoint: "loans", idField: "loan_id",
    displayFields: (item) => [
  ...
  },
  {
    ...
    endpoint: "repayments", idField: "repayment_id",
    displayFields: (item) => [
  ...
  {
    key: "psp-settlements", label: "PSP Settlements", icon: CreditCard,
    endpoint: "psp-settlements", idField: "settlement_id",
    displayFields: (item) => [
      { label: "ID", value: item.settlement_id },
...
function ReinstateTab({ tabCfg }) {
  ...
  const fetchItems = useCallback(async (p, ps, q) => {
    setLoading(true);
    try {
      const params = new URLSearchParams({ page: p, page_size: ps });
      if (q) params.set("search", q);
      const res = await fetch(`${API_URL}/api/reinstate/${tabCfg.endpoint}?${params.toString()}`, { headers: getAuthHeaders() });
  ...
  useEffect(() => { fetchItems(page, pageSize, search); }, [fetchItems, page, pageSize, search]);

  const handleSearchChange = (val) => {
    setSearchInput(val);
    clearTimeout(debounceRef.current);
    debounceRef.current = setTimeout(() => { setPage(1); setSearch(val.trim()); }, 400);
  };
  ...
  const handleReinstateClick = async (item) => {
    ...
    try {
      const res = await fetch(`${API_URL}/api/reinstate/${tabCfg.endpoint}/${id}/preview`, { headers: getAuthHeaders() });
  ...
  const handleCaptchaVerified = async () => {
    ...
    try {
      const res = await fetch(`${API_URL}/api/reinstate/${tabCfg.endpoint}/${id}`, { method: "POST", headers: getAuthHeaders() });
      const data = await res.json();
  ...
  const closeConfirm = () => { setConfirmData(null); setShowCaptcha(false); };
    ...
    <>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 text-slate-400 gap-3">
          ...
          <p className="text-sm">{search ? `No results for "${search}"` : "No items available for reinstatement"}</p>
        </div>
      ) : (
        <div className="space-y-2">
          {items.map((item) => {
            ...
            return (
              <div key={id} className="bg-white border border-slate-200 rounded p-4 flex items-start justify-between gap-4 hover:border-orange-200 transition-colors">
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-x-6 gap-y-1 flex-1">
                  {fields.map((f) => (
                    <div key={f.label}>
                      <p className="text-slate-400 text-xs uppercase tracking-wider">{f.label}</p>
                      <p className="text-slate-800 text-sm font-mono truncate">{f.value}</p>
                    </div>
                  ))}
                </div>
                <Button
      ...
      <Dialog open={!!confirmData && !showCaptcha} onOpenChange={(open) => { if (!open) closeConfirm(); }}>
        <DialogContent className="bg-white border-slate-200 text-slate-800 max-w-xl">
          ...
          <div className="flex gap-3 pt-2 border-t border-slate-200">
            <Button variant="outline" className="flex-1 border-slate-200 text-slate-600" onClick={closeConfirm}>
              Cancel
            </Button>
            <Button className="flex-1 bg-orange-500 hover:bg-orange-600 text-white" onClick={() => setShowCaptcha(true)}>
              Proceed
            </Button>
          </div>
        </DialogContent>
      ...
      <Dialog open={showCaptcha} onOpenChange={(open) => { if (!open) closeConfirm(); }}>
        <DialogContent className="bg-white border-slate-200 text-slate-800 max-w-md">
          <DialogHeader>
            <DialogTitle className="text-orange-500">Verify to Reinstate</DialogTitle>
          </DialogHeader>
          <MathCaptcha onVerified={handleCaptchaVerified} onCancel={closeConfirm} />
        </DialogContent>
      </Dialog>
    </>
  );
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden p-6 gap-6">
      ...
      <div className="flex items-center gap-3 shrink-0">
        ...
        <div>
          <h1 className="text-2xl font-bold text-slate-800">Reinstate Center</h1>
          <p className="text-slate-500 text-sm">Reverse approved records back to pending status. Admin only.</p>
        </div>
        <Badge className="ml-auto bg-orange-100 text-orange-600 border-orange-200">Admin Only</Badge>
      </div>
